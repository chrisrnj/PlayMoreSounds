...
public final class PlayerDeathListener extends SinglePMSListener implements Listener {
    private final @NotNull SoundManager<Player, Entity> soundManager;

    public PlayerDeathListener(@NotNull PlatformUtil platformUtil, @NotNull SoundManager<Player, Entity> soundManager) {
        super(platformUtil, "Player Death");
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        if (!event.isCancelled() || !sound.cancellable()) {
            soundManager.play(sound, event.getEntity());
        }
    }
}
