...
public final class EntityJumpListener extends SinglePMSListener implements Listener {
    private final @NotNull SoundManager<Player, Entity> soundManager;

    public EntityJumpListener(@NotNull PlatformUtil platformUtil, @NotNull SoundManager<Player, Entity> soundManager) {
        super(platformUtil, "Entity Jump");
    ...
    public void onEntityJump(EntityJumpEvent event) {
        if (!event.isCancelled() || !sound.cancellable()) {
            soundManager.play(sound, Emitter.follow(event.getEntity()));
        }
    }
}
