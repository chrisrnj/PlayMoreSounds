...
public final class WeatherRainListener extends MultiplePMSListener implements Listener {
    private final @NotNull SoundManager<Player, Entity> soundManager;

    public WeatherRainListener(@NotNull PlatformUtil platformUtil, @NotNull SoundManager<Player, Entity> soundManager) {
        super(platformUtil, "Weather Rain", "Weather Rain End");
        this.soundManager = soundManager;
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onWeatherChange(WeatherChangeEvent event) {
        PMSSoundList sound = event.toWeatherState() ? sounds[0] : sounds[1];

        if (sound != null && (!event.isCancelled() || !sound.cancellable())) {
            Location loc = event.getWorld().getSpawnLocation();
            soundManager.play(sound, Emitter.loc(event.getWorld().getName(), loc.x(), loc.y(), loc.z()));
        }
    }
}
