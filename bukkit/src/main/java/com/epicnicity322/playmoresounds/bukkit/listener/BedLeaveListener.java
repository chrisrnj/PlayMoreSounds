...
public final class BedLeaveListener extends MultiplePMSListener implements Listener {
    private final @NotNull SoundManager<Player, Entity> soundManager;

    public BedLeaveListener(@NotNull PlatformUtil platformUtil, @NotNull SoundManager<Player, Entity> soundManager) {
        super(platformUtil, "Bed Leave", "Wake Up");
    ...
    public void onBedLeave(PlayerBedLeaveEvent event) {
        ...
        if (sounds[0] != null && (!event.isCancelled() || !sounds[0].cancellable()))
            soundManager.play(sounds[0], player);

        if (sounds[1] != null && (!event.isCancelled() || !sounds[1].cancellable())) {
            long time = player.getWorld().getTime();

            if (time < 300) soundManager.play(sounds[1], player);
        }
    }
}
