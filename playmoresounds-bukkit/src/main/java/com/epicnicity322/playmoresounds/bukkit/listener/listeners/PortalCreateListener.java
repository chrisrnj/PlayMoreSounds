...
public final class PortalCreateListener extends SinglePMSListener {
    public PortalCreateListener(@NotNull PlayMoreSoundsPlugin plugin) {
        super(plugin, "Portal Create");
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onPortalCreate(PortalCreateEvent event) {
        if (!event.isCancelled() || !sound.cancellable()) {
            PlayMoreSoundsPlugin.soundManager().play(sound, event.getBlocks().get(0).getLocation());
        }
    }
}
