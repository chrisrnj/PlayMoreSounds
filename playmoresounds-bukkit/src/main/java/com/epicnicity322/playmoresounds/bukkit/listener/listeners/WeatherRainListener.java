...
public final class WeatherRainListener extends MultiplePMSListener {
    public WeatherRainListener(@NotNull PlayMoreSoundsPlugin plugin) {
        super(plugin, new String[]{"Weather Rain", "Weather Rain End"}, Configurations.SOUNDS);
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onWeatherChange(WeatherChangeEvent event) {
        Sound sound = event.toWeatherState() ? sounds[0] : sounds[1];

        if (sound != null && (!event.isCancelled() || !sound.cancellable())) {
            PlayMoreSoundsPlugin.soundManager().play(sound, event.getWorld().getSpawnLocation());
        }
    }
}
