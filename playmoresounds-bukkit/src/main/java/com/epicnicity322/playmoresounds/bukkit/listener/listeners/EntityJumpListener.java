...
public final class EntityJumpListener extends SinglePMSListener {
    public EntityJumpListener(@NotNull PlayMoreSoundsPlugin plugin) {
        super(plugin, "Entity Jump");
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onEntityJump(EntityJumpEvent event) {
        if (!event.isCancelled() || !sound.cancellable()) {
            PlayMoreSoundsPlugin.soundManager().play(sound, event.getEntity().getLocation());
        }
    }
}
